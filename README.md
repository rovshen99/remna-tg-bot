...
- `ADMIN_NOTIFICATIONS_CHAT_ID` — ID чата/канала, куда слать админские уведомления (опционально)
...
- `HWID_DEVICE_LIMIT_PRESETS` — варианты для кнопок выбора лимита устройств (через запятую), по умолчанию `1` (для дилеров)
