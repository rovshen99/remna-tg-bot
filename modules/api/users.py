...
class UserAPI:
    ...
    async def delete_all_user_hwid_devices(uuid):
        """Delete all HWID devices bound to a user via the panel's bulk endpoint"""
        result = await RemnaAPI.post("hwid/devices/delete-all", {"userUuid": uuid})
        if not isinstance(result, dict):
            logger.error(f"Failed to delete all HWID devices for user {uuid}")
            return {"total": 0, "deleted": 0}

        total = result.get("total", len(result.get("devices", [])))
        return {"total": total, "deleted": total}
